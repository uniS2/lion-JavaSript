...
//? 바로 실행 되는게 아닌, 사용자가 어떤 인터렉션(행동)을 했을 때 코드 로직이 실행되는 경우
//? 완전 중단이 아닌 대안들. 예외처리
try{
  // let a = 5;
  // a + b;
...
//? 이러한 오류 유형을 "런타임 오류" 또는 "예외(exception)"라고 부름 => try..catch문 "예외처리"
...
// throw new SyntaxError("참조되지 않았습니다")
...
// 객체처럼 보이나 문자. *서버에서 값 전달받을 경우 대부분 문자로 값 들어온다.
let data = JSON.stringify({ // 내장된 문자반환 객체(?)
  "id": 1,
  "name": "Leanne Graham",
  "username": "Bret",
...
try{
  ...
  const user = JSON.parse(data)  // parse: '해석한다'. 원래의 자료형 값이 됨.

  console.log(user.value);  // 없는 요소에도 접근가능.
  ...
  document.body.innerHTML = '404 not found' // 좀 위험
}
finally {
